...
function StatusBadge({ status, chanceOfPlaying }: { status: string; chanceOfPlaying: number | null }) {
  ...
  return (
    <span style={{
      ...
      padding: '1px 5px',
      borderRadius: '3px',
      background: isInjured ? 'rgba(255,80,80,0.15)' : 'rgba(255,180,0,0.15)',
      color: isInjured ? '#ff5050' : '#ffb400',
      flexShrink: 0,
    }}>
      {isInjured ? 'INJ' : chanceOfPlaying !== null ? `${chanceOfPlaying}%` : 'OUT'}
    </span>
  )
}
...
function CaptainCard({ pick, index }: { pick: CaptainPick; index: number }) {
  ...
  const highForm = pick.fplForm ? parseFloat(pick.fplForm) >= 6 : false
    ...
    <div style={{
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div style={{ display: 'flex', gap: '0px', marginBottom: '6px' }}>
          <StatCell label="Price" value={pick.fplPrice ?? '—'} />
          <StatCell label="Own%"  value={pick.fplOwnership ?? '—'} />
          <StatCell label="Form"  value={pick.fplForm ?? '—'} highlight={highForm} />
          <StatCell label="Pts"   value={pick.fplTotalPoints ?? '—'} />
          <StatCell label="Pts/£" value={pick.pointsPerCost ?? '—'} />
        </div>

        {/* Stats row */}
        <div style={{ display: 'flex', gap: '0px' }}>
          <StatCell label="xGoals"  value={pick.xGPer90} />
          <StatCell label="xAssists"  value={pick.xAPer90} />
          <StatCell label="xDefCon" value={pick.defConPerGame} />
          <StatCell label="Pen"    value={pick.isPenaltyTaker  ? '✓' : '—'} highlight={pick.isPenaltyTaker} />
          <StatCell label="Set Pieces"    value={pick.isSetPieceTaker ? '✓' : '—'} highlight={pick.isSetPieceTaker} />
        </div>
      </div>
...
export default function FPLFixtureDifficulty({ teams, upcomingGWs, captainPicks, nextGW }: Props) {
  ...
  const visiblePicks = hideUnavailable
    ? captainPicks.filter(p =>
        p.status === 'a' &&
        (p.chanceOfPlaying === null || p.chanceOfPlaying > 50)
      )
    : captainPicks
    ...
    <>
      ...
      <div className="app">
        ...
        <div className="toggle-bar">
          <button className={`toggle-btn ${mode === 'fdr'     ? 'active' : ''}`} onClick={() => setMode('fdr')}>Fixture Difficulty</button>
          <button className={`toggle-btn ${mode === 'cs'      ? 'active' : ''}`} onClick={() => setMode('cs')}>Clean Sheet %</button>
          <button className={`toggle-btn ${mode === 'captain' ? 'active' : ''}`} onClick={() => setMode('captain')}>Captain Picks</button>
        </div>
          ...
          <div style={{ padding: '8px 16px 4px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            ...
            <button
              ...
              style={{
                padding: '5px 10px',
                borderRadius: '20px',
                border: `1px solid ${hideUnavailable ? '#00c864' : '#1a2030'}`,
                background: hideUnavailable ? 'rgba(0,200,100,0.1)' : '#0e1318',
                color: hideUnavailable ? '#00c864' : '#8896a8',
                fontSize: '10px',
                fontWeight: 600,
                cursor: 'pointer',
              ...
              <tbody>
                {(() => {
                  const sorted = [...teams].sort((a, b) => {
                    if (mode === 'fdr') {
                      const avgDiffA = a.fixtures.reduce((s, f) => s + f.difficulty, 0) / (a.fixtures.length || 1)
                      const avgDiffB = b.fixtures.reduce((s, f) => s + f.difficulty, 0) / (b.fixtures.length || 1)
                      if (avgDiffA !== avgDiffB) return avgDiffA - avgDiffB
                      const avgCSA = a.fixtures.reduce((s, f) => s + f.csChance, 0) / (a.fixtures.length || 1)
                      const avgCSB = b.fixtures.reduce((s, f) => s + f.csChance, 0) / (b.fixtures.length || 1)
